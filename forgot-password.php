...
if (isset($_POST["forgot-password"])) {
    ...
    $Npassword = trim(md5($_POST["Npassword"]));
    $cNpassword = trim(md5($_POST["cNpassword"]));
    ...
    if (empty($_POST["Npassword"])) {
        $error['Npassword'] = 'Vui lòng nhập mật khẩu mới !';
    } else if (!isPassword($_POST["Npassword"])) {
        $error['Npassword'] = 'Mật khẩu phải đảm bảo: độ dài 8-32 ký tự, ký tự đầu tiên viết hoa, có ít nhất 1 chữ cái (a-z) hoặc 1 chữ số hoặc 1 ký tự đặc biệt !';
    } else if ($_POST["Npassword"] == $usernameInp) {
        $error['password'] = 'Không trùng với tên tài khoản !';
    }

    if (empty($_POST["cNpassword"])) {
        $error['cNpassword'] = 'Vui lòng nhập lại mật khẩu mới !';
    }

    if ($_POST["Npassword"] != $_POST["cNpassword"]) {
        $error['cNpassword'] = 'Nhập lại mật khẩu mới không chính xác !';
    }

    if (!$error) {
        $sql = "SELECT *
								FROM loginsignup
								WHERE username = '$usernameInp' AND email = '$email'";
        $result = mysqli_query($conn, $sql);
        if ($result->num_rows > 0) {
            $sql = "UPDATE loginsignup
            ...
            header("Location: forgot-password.php?error=Khôi phục mật khẩu thất bại: sai tên tài khoản hoặc địa chỉ email. Vui lòng thử lại !");
        }
    }
}
...
?></li>
	...
	<div class="main-container login">
		<img src="assets/img/banner-website-shopbongda-Nike-CR7-xanh-lơ-3.png"
			alt="" class="background" style="height: 91rem; ">
		<div class="grid wide">
			<div class="row">
				<div class="col l-6 form" style="height: 81rem; ">
					<div class="row"
						style="display: flex; justify-content: center; margin-top: -2rem;	">
						<div class="category-archives"><span>Quên mật khẩu</span></div>
					</div>
					<div class="row">
						<form action="" class="form-wrapper" id="forgot-password"
							method="POST">
							<?php if ($_GET['error']) {?>
							...
							<div class="form-group">
								...
								<input type="email" id="email" class="form-wrapper__input"
							...
							<div class="form-group form-group-password">
								...
								<input type="password" id="Npassword"
									class="form-wrapper__input" name="Npassword"
									placeholder="Nhập mật khẩu mới..."
									value="<?php echo $_POST['Npassword']; ?>">
								<span class="message-error">
									<?php echo isset($error['Npassword']) ? $error['Npassword'] : ''; ?>
								</span>
							...
							<div class="form-group form-group-Cpassword">
								<label for="confirm-password">Nhập lại mật khẩu mới:</label><br>
								<input type="password" id="confirm-Npassword"
									class="form-wrapper__input" name="cNpassword"
									placeholder="Nhập lại mật khẩu mới..."
									value="<?php echo $_POST['cNpassword']; ?>">
								<span class="message-error">
									<?php echo isset($error['cNpassword']) ? $error['cNpassword'] : ''; ?>
								</span>
							...
							<input type="submit" value="Khôi phục mật khẩu" class="btn"
								name='forgot-password' id="forgot-password-btn">
...
?>
	...
	<script src="assets/js/validate-forgot-password.js"></script>
	<script>
	Validator({
		...
		rules: [
			...
			Validator.isRequired("#Npassword", "Vui lòng nhập mật khẩu mới !"),
			...
			Validator.isConfirmed(
				"#confirm-Npassword",
				() => {
					return document.querySelector("#forgot-password #Npassword")
						.value;
				},
	...
	var cpassword = document.getElementById("confirm-Npassword");
