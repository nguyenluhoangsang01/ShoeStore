...
if (isset($_POST["signup"])) {
    ...
    if (empty($fullname)) {
        ...
        $error['class'] = 'invalid';
    } else if (!isInvalidName($fullname)) {
        $error['fullname'] = 'Họ và tên không hợp lệ !';
        $error['class'] = 'invalid';
    }

    if (empty($usernameInp)) {
        $error['username'] = 'Vui lòng nhập tên tài khoản !';
        $error['class'] = 'invalid';
    } else if (!isInvalidCharacters($usernameInp)) {
        $error['username'] = 'Tên tài khoản phải đảm bảo: độ dài 8-32 ký tự, bắt đầu bằng chữ cái (a-z) hoặc (A-Z) và 1 chữ số !';
        $error['class'] = 'invalid';
    }
    ...
    if (empty($_POST["password"])) {
        ...
        $error['password'] = 'Mật khẩu phải đảm bảo: độ dài 8-32 ký tự, ký tự đầu tiên viết hoa, có ít nhất 1 chữ cái (a-z) hoặc 1 chữ số hoặc 1 ký tự đặc biệt !';
        $error['class'] = 'invalid';
    } else if ($_POST["password"] == $usernameInp) {
        $error['password'] = 'Không trùng với tên tài khoản !';
        $error['class'] = 'invalid';
    }

    if (empty($_POST["cpassword"])) {
        $error['cpassword'] = 'Vui lòng nhập lại mật khẩu !';
        $error['class'] = 'invalid';
    }

    if ($_POST["password"] != $_POST["cpassword"]) {
        $error['cpassword'] = 'Nhập lại mật khẩu không chính xác !';
        $error['class'] = 'invalid';
    }
    ...
    if (!$error) {
        $sql = "	SELECT *
									FROM loginsignup
									WHERE username = '$usernameInp'";
        $result = mysqli_query($conn, $sql);

        if ($result->num_rows > 0) {
            echo "<script type='text/javascript'>alert('Tên tài khoản đã tồn tại !')</script>";
        } else {
            $sql = "INSERT INTO loginsignup (fullname, username, phone, email, password)
										VALUES ('$fullname','$usernameInp','$phone','$email','$password')";
            $result = mysqli_query($conn, $sql);
            if ($result) {
                header("Location: login.php");
            } else {
...
?></li>
	...
	<div class="main-container login">
		<img
			src="assets/img/Converse_x_Pop_Trading_Company_desktop_banner_-1_1200x500_crop_center.progressive.jpg"
			alt="" class="background" style="height: 135rem; ">
		<div class="grid wide">
			<div class="row">

				<div class="col l-6 form" style="height: 125rem; ">
					<div class="row"
						style="display: flex; justify-content: center; margin-top: -2rem;	">
						<div class="category-archives"><span>Tạo tài khoản</span></div>
					</div>
					<div class="sub-heading">Hãy điền đầy đủ thông tin bên dưới để đăng ký
						tài khoản thành viên </div>

					<div class="row">
						<form action="" method="POST" class="form-wrapper" id="form-signup">
							<div class="form-group">
								<label for="full-name">Họ và tên:</label> <br>

								<?php isset($error['class']);{?>
								<input type="text" id="full-name"
									class="form-wrapper__input <?=$error['class']?>"
							...
							<div class="form-group">
								...
								<input type="text" id="phone"
									class="form-wrapper__input <?=$error['class']?>" name="phone"
							...
							<div class="form-group">
								...
								<input type="email" id="email"
									class="form-wrapper__input <?=$error['class']?>" name="email"
							...
							<p
								style="font-size: 1.5rem; text-align: center; margin-bottom: 2rem;margin-top: 3rem;line-height: 1.4;">
								Bằng việc đăng kí, bạn đã đồng ý với THUTHUYSHOESTORE về
								<a href="#"
									style="color: var(--primary-color); font-weight: 500;">Điều
									khoản dịch vụ</a> &
								<a href="#"
									style="color: var(--primary-color); font-weight: 500;">Chính
									sách bảo mật</a>
							</p>
							...
							<input type="submit" value="Đăng ký" class="btn" name="signup"
								style="margin-bottom: 3rem; " id='signup'>
							<p style="text-align: center; ">
								Bạn đã có tài khoản ? <a href=" login.php" class="text">Đăng
									nhập</a>.</p>
						</form>
					</div>
				</div>
			...
			<div class="row">
				<div class="wrapper-main-wap">
					<div class="main-container main-content">
						<div class="row"
							style="display: flex; justify-content: center; margin-top: -21rem;	">
							<div class="category-archives"><span>Tin tức</span></div>
						</div>

						<div class="product-container">
							<div class="grid wide">
								<div class="row" style="margin-top: -27rem;">
									<div class="col l-3">
										<div class="product">
...
?>
	...
	<script src="assets/js/validate-signup-form.js"></script>
	<script>
	Validator({
		form: "#form-signup",
		...
		rules: [
			Validator.isRequired("#full-name", "Vui lòng nhập họ và tên !"),
			Validator.isInvalidName("#full-name",
				"Họ và tên không hợp lệ !"),

			Validator.isRequired("#user-name",
				"Vui lòng nhập tên tài khoản !"),
			Validator.isInvalidCharacters("#user-name",
				"Tên tài khoản phải đảm bảo: độ dài 8-32 ký tự, bắt đầu bằng chữ cái (a-z) hoặc (A-Z) và 1 chữ số !"
			),
			...
			Validator.isPassword(
				"#password",
				"Mật khẩu phải đảm bảo: độ dài 8-32 ký tự, ký tự đầu tiên viết hoa, có ít nhất 1 chữ cái (a-z) hoặc 1 chữ số hoặc 1 ký tự đặc biệt !"
			),
	...
	// Nhập lại mật khẩu
	var btnShowCPassword = document.getElementById("btnShowCPassword");
	var btnHideCPassword = document.getElementById("btnHideCPassword");
	var cpassword = document.getElementById("confirm-password");

	btnHideCPassword.addEventListener("click", () => {
		btnShowCPassword.style.opacity = "1";
		btnShowCPassword.style.visibility = "visible";

		btnHideCPassword.style.opacity = "0";
		btnHideCPassword.style.visibility = "hidden";

		cpassword.type = "text";
		cpassword.focus();
	});

	btnShowCPassword.addEventListener("click", () => {
		btnHideCPassword.style.opacity = "1";
		btnHideCPassword.style.visibility = "visible";

		btnShowCPassword.style.opacity = "0";
		btnShowCPassword.style.visibility = "hidden";
